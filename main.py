...
#                      VERSION: 6.17 (STABLE RESTORE + BACKGROUND WORKER)      #
...
from telegram import (
    ...
    ReplyKeyboardRemove, 
    constants,
    LinkPreviewOptions  # FITUR BARU: Mematikan preview link WA
)
...
# --- DIAGNOSTIC STARTUP ---
print("\n" + "="*50)
print("🔍 SYSTEM DIAGNOSTIC STARTUP (v6.17)")
print("="*50)
...
# ##############################################################################
# BAGIAN 7: BACKGROUND UPLOAD & ADMIN HELPERS (THE FIX)
# ##############################################################################

# [NEW] Worker Process untuk Upload di Background
    ...
    try:
        for i in range(0, total, BATCH_SIZE):
            if context.user_data.get('stop_signal'):
                await status_msg.edit_text("⛔ <b>DIHENTIKAN USER.</b>", reply_markup=None)
                return
            ...
            # Update Progress (Tiap 200 data atau 10%)
            if i % 200 == 0 and i > 0:
                pct = int((i/total)*100)
                try:
        ...
        err_info = f"\n⚠️ Error: {errors[0]}" if errors else ""
    ...
    try:
        ...
        if pic_list:
            ...
            for i, u in enumerate(pic_list, 1):
                entry = f"{i}. 🤝 <b>{clean_text(u.get('nama_lengkap'))}</b>\n   📱 {format_wa_link(u.get('no_hp'))} | 🏢 {clean_text(u.get('agency'))}\n   ⚙️ /m_{u['user_id']}\n\n"
...
async def auto_cleanup_logs(context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        print(f"🧹 [AUTO CLEANUP] Log lama dihapus.")
...
async def notify_leasing_group(context, matel_user, unit_data):
    ...
    try:
        ...
        msg_group = (
            ...
            f"⚙ <b>Nosin:</b> {clean_text(unit_data.get('nosin'))}\n"
            f"----------------------------------\n"
...
# ==============================================================================
# BAGIAN 10: UPLOAD SYSTEM (REPLACED WITH BACKGROUND WORKER)
# ==============================================================================
...
async def register_confirm(update, context):
    ...
    try:
        ...
        logger.error(f"Reg Error: {e}"); await update.message.reply_text("❌ Gagal Terkirim. User ID Anda mungkin sudah terdaftar sebelumnya.", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
async def handle_message(update, context):
    ...
    if txt == "🔄 SINKRONISASI DATA": return await upload_start(update, context)
    if txt == "📂 DATABASE SAYA": return await cek_kuota(update, context)
    ...
    kw = re.sub(r'[^a-zA-Z0-9]', '', txt.upper())
...
# ==============================================================================
# BAGIAN 13: HANDLER KONVERSASI (TAMBAH, LAPOR)
# ==============================================================================
...
if __name__ == '__main__':
    print("🚀 ONEASPAL BOT v6.17 (STABLE RESTORE + BACKGROUND WORKER) STARTING...")
    ...
    # --- [UPGRADE] UPLOAD HANDLER DENGAN BACKGROUND WORKER ---
    app.add_handler(ConversationHandler(
    ...
    print("✅ BOT ONLINE! (v6.17 - STABLE BASE)")
    app.run_polling()