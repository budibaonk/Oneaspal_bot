...
async def notify_hit_to_group(context, u, d):
    try:
        # Cek apakah LOG_GROUP_ID sudah diisi di .env
        if LOG_GROUP_ID == 0: return

        # Siapkan Nomor WA Bersih untuk tombol
        hp_raw = str(u.get('no_hp', ''))
        clean_num = re.sub(r'[^0-9]', '', hp_raw)
        if clean_num.startswith('0'): clean_num = '62' + clean_num[1:]
...
# 1. FUNGSI KERJA DI LATAR BELAKANG (AGAR BOT TIDAK STUCK)
async def background_upload_process(update, context, act, data, chat_id):
    # Tombol Panic Button (STOP)
...
# 4. HANDLER PROSES DATA (CLEANING & PREVIEW)
async def upload_leasing_admin(update, context):
    try:
        ...
        # --- PREVIEW DINAMIS (Tampilkan semua kolom yang ada) ---
        s = final_data[0] 
        details_list = []
