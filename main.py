...
def read_file_robust(file_content, file_name):
    ...
    # Strategi CSV: Coba kombinasi encoding & separator
...
async def admin_topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if success:
            await update.message.reply_text(f"✅ **TOPUP SUKSES**\nUser ID: `{target_id}`\nTambah: +{amount}\nTotal: {new_balance}", parse_mode='Markdown')
            try: await context.bot.send_message(chat_id=target_id, text=f"🎉 **KUOTA BERTAMBAH!**\nAdmin menambah +{amount} kuota.\nTotal: {new_balance}")
            except: pass
        else:
    ...
    # ALUR 2: ADMIN (SMART PROCESSING)
        ...
        try:
            ...
            if 'nopol' not in df.columns:
                ...
                await msg.delete()
                await update.message.reply_text(f"❌ **GAGAL DETEKSI NOPOL**\nKolom terbaca: {det}\nPastikan ada kolom: 'No Polisi'.")
            ...
            report = (
                ...
                f"━━━━━━━━━━━━━━━━━━\n\n"
                f"👉 **MASUKKAN NAMA LEASING:**\n_(Ketik 'SKIP' jika menggunakan kolom dari file)_"
            )
            
            # Hapus pesan lama, kirim pesan baru (untuk menghindari error keyboard)
            await msg.delete()
            await update.message.reply_text(report, parse_mode='Markdown', reply_markup=ReplyKeyboardMarkup([["SKIP"], ["❌ BATAL"]], resize_keyboard=True, one_time_keyboard=True))
...
async def contact_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg_content: return await update.message.reply_text("⚠️ Contoh: `/admin Lapor error...`", parse_mode='Markdown')
    try:
        await context.bot.send_message(chat_id=ADMIN_ID, text=f"📩 **PESAN MITRA**\n👤 {u.get('nama_lengkap')}\n📱 `{u.get('user_id')}`\n💬 {msg_content}")
        await update.message.reply_text("✅ Terkirim ke Admin.")
    except: await update.message.reply_text("❌ Gagal mengirim pesan.")
...
async def panduan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "6️⃣ **UPLOAD:** Kirim file Excel ke chat bot."
    )
...
if __name__ == '__main__':
    ...
    print("✅ ONEASPAL BOT ONLINE - V3.0 (HR MANAGER & FULL LEGACY)")
    app.run_polling()