...
async def register_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Terkirim! Tunggu aktivasi Admin.", reply_markup=ReplyKeyboardRemove())
...
# ==============================================================================
#                     USER: TAMBAH DATA (FIXED NAME)
# ==============================================================================

async def add_data_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = get_user(update.effective_user.id)
    if not user or user['status'] != 'active': return await update.message.reply_text("⛔ Akun tidak aktif.")
...
async def add_nokir(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📋 **KONFIRMASI**\nNopol: {context.user_data['add_nopol']}", reply_markup=ReplyKeyboardMarkup([["✅ KIRIM"]], one_time_keyboard=True))
    return A_CONFIRM

# NAMA FUNGSI INI SEBELUMNYA SALAH (FIXED)
async def add_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    nopol = context.user_data['add_nopol']
    context.bot_data[f"prop_{nopol}"] = {"nopol": nopol, "type": context.user_data['add_type'], "finance": context.user_data['add_leasing'], "ovd": f"Kiriman: {context.user_data['add_nokir']}"}
    await update.message.reply_text("✅ Dikirim ke Admin.", reply_markup=ReplyKeyboardRemove())
    kb = [[InlineKeyboardButton("✅ Terima", callback_data=f"v_acc_{nopol}_{update.effective_user.id}"), InlineKeyboardButton("❌ Tolak", callback_data="v_rej")]]
    await context.bot.send_message(ADMIN_ID, f"📥 **DATA BARU:** {nopol}", reply_markup=InlineKeyboardMarkup(kb))
    return ConversationHandler.END
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("appu_"):
        ...
        _, _, nopol, uid = query.data.split("_"); item = context.bot_data.get(f"prop_{nopol}")
        if item:
            supabase.table('kendaraan').upsert(item).execute()
            await query.edit_message_text(f"✅ {nopol} Masuk Database.")
            await context.bot.send_message(uid, f"🎊 Data `{nopol}` disetujui!")
...
if __name__ == '__main__':
    ...
    # PERBAIKAN DI SINI: A_CONFIRM memanggil add_confirm
