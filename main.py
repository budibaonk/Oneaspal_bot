...
#                      VERSION: 6.19 (FULL CODE INTEGRITY - NO COMPRESSION)    #
...
# --- DIAGNOSTIC STARTUP ---
print("\n" + "="*50)
print("🔍 SYSTEM DIAGNOSTIC STARTUP (v6.19)")
print("="*50)
...
# ##############################################################################
# BAGIAN 3: DEFINISI STATE CONVERSATION (JANGAN DIHAPUS)
# ##############################################################################
...
def check_subscription_access(user):
    try:
        ...
        today_str = datetime.now(TZ_JAKARTA).strftime('%Y-%m-%d')
...
def increment_daily_usage(user_id, current_usage):
    try: supabase.table('users').update({'daily_usage': current_usage + 1}).eq('user_id', user_id).execute()
    except: pass
...
# ##############################################################################
# BAGIAN 7: BACKGROUND UPLOAD & STOP (THE FIX FOR STUCK UPLOAD)
# ##############################################################################
    ...
    try:
        ...
        for key in priority_cols:
            ...
            if val and str(val).strip() not in ['-', 'nan', 'None', '', 'UNKNOWN']:
                if key == 'nopol': val = f"<code>{val}</code>"
...
# ##############################################################################
# BAGIAN 9: USER FEATURES (REGISTER, START, DLL)
# ##############################################################################
...
async def register_confirm(update, context):
    ...
    try:
        ...
        await context.bot.send_message(ADMIN_ID, msg_admin, reply_markup=InlineKeyboardMarkup(kb), parse_mode='HTML', link_preview_options=no_preview)
    except Exception as e: 
        logger.error(f"Reg Error: {e}"); await update.message.reply_text("❌ Gagal Terkirim.", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
    ...
    try:
        ...
        if pic_list:
            ...
            for i, u in enumerate(pic_list, 1):
                nama = clean_text(u.get('nama_lengkap')); agency = clean_text(u.get('agency')); wa_link = format_wa_link(u.get('no_hp')); uid = u['user_id']
                entry = (f"{i}. 🤝 <b>{nama}</b>\n   📱 {wa_link} | 🏢 {agency}\n   ⚙️ /m_{uid}\n\n")
                if len(msg) + len(entry) > 4000: await update.message.reply_text(msg, parse_mode='HTML', link_preview_options=no_preview); msg = ""
                msg += entry
            msg += "━━━━━━━━━━━━━━━━━━\n"
        if field_list:
            msg += "🛡️ <b>MITRA LAPANGAN</b>\n"
            for i, u in enumerate(field_list, 1):
                role = u.get('role', 'matel'); icon = "🎖️" if role == 'korlap' else "🛡️"; nama = clean_text(u.get('nama_lengkap')); agency = clean_text(u.get('agency')); wa_link = format_wa_link(u.get('no_hp')); uid = u['user_id']
...
async def callback_handler(update, context):
    ...
    if data == "stop_upload_task":
        ...
        nopol = data.replace("view_", ""); u = get_user(update.effective_user.id)
        ...
        uid = int(data.split("_")[2]); supabase.table('users').update({'role': 'matel'}).eq('user_id', uid).execute()
...
async def show_unit_detail_original(update, context, d, u):
    ...
    txt = (f"{info_txt}✅ <b>DATA DITEMUKAN</b>\n━━━━━━━━━━━━━━━━━━\n🚙 <b>Unit:</b> {clean_text(d.get('type'))}\n🔢 <b>Nopol:</b> <code>{clean_text(d.get('nopol'))}</code>\n📅 <b>Tahun:</b> {clean_text(d.get('tahun'))}\n🎨 <b>Warna:</b> {clean_text(d.get('warna'))}\n----------------------------------\n🔧 <b>Noka:</b> <code>{clean_text(d.get('noka'))}</code>\n⚙️ <b>Nosin:</b> <code>{clean_text(d.get('nosin'))}</code>\n----------------------------------\n⚠️ <b>OVD:</b> {clean_text(d.get('ovd'))}\n🏦 <b>Finance:</b> {clean_text(d.get('finance'))}\n🏢 <b>Branch:</b> {clean_text(d.get('branch'))}\n━━━━━━━━━━━━━━━━━━\n⚠️ <b>CATATAN PENTING:</b>\n<i>Ini bukan alat yang SAH untuk penarikan. Konfirmasi ke PIC leasing.</i>")
    await context.bot.send_message(chat_id=update.effective_chat.id, text=txt, parse_mode='HTML')
    await notify_hit_to_group(context, u, d); await notify_leasing_group(context, u, d)
...
if __name__ == '__main__':
    print("🚀 ONEASPAL BOT v6.19 (FULL CODE INTEGRITY) STARTING...")
