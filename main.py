...
#                      VERSION: 6.12 (FULL RESTORED - NO COMPRESSION)          #
...
# --- DIAGNOSTIC STARTUP ---
print("\n" + "="*50)
print("🔍 SYSTEM DIAGNOSTIC STARTUP (v6.12)")
print("="*50)
...
if not URL or not KEY or not TOKEN:
    print("❌ CRITICAL: Credential Database/Bot Hilang!")
    exit()
...
# ##############################################################################
# BAGIAN 2: KAMUS DATA (DICTIONARY)
# ##############################################################################
...
# ##############################################################################
# BAGIAN 4: FUNGSI HELPER UTAMA (UTILITIES)
# ##############################################################################

async def post_init(application: Application):
    """Mengatur menu command di Telegram saat bot start."""
...
def check_subscription_access(user):
    """Mengecek apakah user boleh mengakses bot (Kuota & Expired)."""
    try:
        # PIC Leasing Unlimited
        if user.get('role') == 'pic': 
            return True, "OK"
        ...
        # Cek Kuota Harian (Reset tiap hari)
...
def increment_daily_usage(user_id, current_usage):
    """Menambah counter penggunaan harian."""
    try:
...
def add_subscription_days(user_id, days_to_add):
    """Menambah durasi langganan user."""
    try:
...
def clean_text(text):
    """Membersihkan text agar aman untuk HTML parse mode."""
    if not text: return "-"
    return html.escape(str(text))

def format_wa_link(phone_number):
    """Membuat link WhatsApp yang bisa diklik."""
...
def standardize_leasing_name(name):
    """Standarisasi nama leasing (huruf besar, hapus karakter aneh)."""
...
def log_successful_hit(user_id, user_name, unit_data):
    """Mencatat log ketika unit ditemukan (HIT)."""
    try:
...
# ##############################################################################
# BAGIAN 5: ENGINE FILE & EXCEL PARSING
# ##############################################################################
...
def smart_rename_columns(df):
    """Mengubah nama kolom Excel menjadi standar database."""
    new = {}
    found = []
...
def read_file_robust(content, fname):
    """Membaca file Excel/CSV dengan berbagai encoding."""
...
# ##############################################################################
# BAGIAN 6: FITUR ADMIN - ACTION & APPROVAL
# ##############################################################################
...
async def angkat_korlap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        wilayah = " ".join(context.args[1:]).upper()
...
async def reject_complete(update, context):
    ...
    await update.message.reply_text("✅ Proses Selesai.", reply_markup=ReplyKeyboardRemove())
...
async def admin_action_start(update, context):
    ...
    context.user_data['adm_act_type'] = parts[1]
    context.user_data['adm_act_uid'] = parts[2]
...
async def admin_action_complete(update, context):
    if update.message.text == "❌ BATAL": return await cancel(update, context)
    act = context.user_data.get('adm_act_type')
    ...
    await update.message.reply_text(f"✅ Action {act} Sukses.", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END
...
async def background_upload_process(update, context, act, data, chat_id):
    ...
    try:
        for i in range(0, total, BATCH_SIZE):
            ...
            # Napas biar bot responsif dan tidak diblokir
...
# ##############################################################################
# BAGIAN 9: USER FEATURES (REGISTER, START, DLL) - YANG SEMPAT HILANG
# ##############################################################################
...
async def register_start(update, context):
    ...
    await update.message.reply_text(msg, parse_mode='Markdown', reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True))
    return R_ROLE_CHOICE
...
async def register_role_choice(update, context):
    ...
    if "1️⃣" in choice: 
        ...
        await update.message.reply_text("🛡️ **FORMULIR MITRA LAPANGAN**\n\n1️⃣ Masukkan **Nama Lengkap**:", reply_markup=ReplyKeyboardMarkup([["❌ BATAL"]]))
        return R_NAMA
    elif "2️⃣" in choice: 
...
async def register_email(update, context): 
    ...
    await update.message.reply_text("4️⃣ Kota Domisili:")
    return R_KOTA
...
async def show_unit_detail_original(update, context, d, u):
    ...
    if u.get('role') != 'pic': log_successful_hit(u['user_id'], u.get('nama_lengkap'), d)
...
async def show_multi_choice(update, context, data_list, keyword):
    ...
    for i, item in enumerate(data_list):
        nopol = item['nopol']
        unit = item.get('type', 'Unknown')[:10]
        leasing = item.get('finance', 'Unknown')
...
# ##############################################################################
# BAGIAN 11: MAIN ENTRY POINT (JANTUNG BOT)
# ##############################################################################

if __name__ == '__main__':
    print("🚀 ONEASPAL BOT v6.12 (RESTORED LEGACY) STARTING...")
    app = ApplicationBuilder().token(TOKEN).post_init(post_init).build()
    
    # 1. MESSAGE HANDLER (Callback & Panel)
    app.add_handler(MessageHandler(filters.Regex(r'^/m_\d+$'), manage_user_panel))
    
    # 2. CONVERSATION HANDLERS (Admin & Upload)
    ...
    # 3. CONVERSATION HANDLERS (User Features)
    app.add_handler(ConversationHandler(
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            A_NOKIRIMAN: [MessageHandler(filters.TEXT, add_nokiriman)],
            A_OVD: [MessageHandler(filters.TEXT, add_ovd)],
            A_KET: [MessageHandler(filters.TEXT, add_ket)],
    ...
    app.add_handler(CommandHandler('stop', stop_upload_command)) # NEW STOP
    
    # 5. DYNAMIC HANDLERS (REGEX & PHOTO)
    ...
    # 6. CALLBACK HANDLER (INTERACTION)
    app.add_handler(CallbackQueryHandler(callback_handler))
    
    # 7. MAIN TEXT HANDLER (SEARCH ENGINE) - MUST BE LAST
