...
async def upload_leasing_admin(update, context):
    ...
    if nm != 'SKIP': 
        ...
        fin_display = df['finance'].iloc[0]
    else:
        if 'finance' in df.columns:
            df['finance'] = df['finance'].apply(standardize_leasing_name)
            fin_display = "AUTO (DARI FILE)"
        else:
            df['finance'] = 'UNKNOWN'
            fin_display = "UNKNOWN"

    # 2. MESIN CUCI DATA (NOPOL, NOKA, NOSIN)
    # Penting: Buang spasi liar dan titik yang bikin Supabase Error
    for col in ['nopol', 'noka', 'nosin']:
        if col in df.columns:
            df[col] = df[col].astype(str).str.replace(r'[^a-zA-Z0-9]', '', regex=True).str.upper()
    
    # 3. CLEANING AKHIR
    df = df.drop_duplicates(subset=['nopol'], keep='last')
    df = df.replace({np.nan: "-", "nan": "-", "None": "-", "NAT": "-"})
    
    valid = ['nopol', 'type', 'tahun', 'warna', 'noka', 'nosin', 'ovd', 'finance', 'branch']
    for c in valid: 
        if c not in df.columns: df[c] = "-"
    
    # Simpan data final yang sudah BERSIH
    data_final = df[valid].to_dict(orient='records')
    context.user_data['final_data_records'] = data_final
    
    s = df.iloc[0]
    preview_msg = (
        f"🔎 **PREVIEW DATA SIAP UPLOAD**\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"🏦 **Mode:** {fin_display}\n"
        f"📊 **Total:** {len(data_final)} Data\n\n"
        ...
        f"🔹 OVD: {s['ovd']}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"⚠️ Klik **🚀 EKSEKUSI** untuk simpan ke database."
    )
...
async def upload_confirm_admin(update, context):
    """
    JALUR EKSEKUSI FINAL (STABLE & CLEAN)
    Menghilangkan kata 'Latar Belakang' dan menggunakan batching langsung.
    """
    act = update.message.text
    if act == "❌ BATAL": return await cancel(update, context)
    if act not in ["🚀 EKSEKUSI", "🚀 UPDATE DATA"]: return
    ...
    msg_status = await update.message.reply_text(
        "🚀 **MEMULAI PROSES SIMPAN...**\n🧹 _Membersihkan data & menanam ke database..._", 
        parse_mode='Markdown', 
        reply_markup=ReplyKeyboardRemove()
    )
    ...
    start_time = time.time()
    try:
        # Batching Sederhana (80 per tembakan)
        # Menangani data kotor otomatis dari tahap sebelumnya
        batch1 = data[:80]
        batch2 = data[80:]

        # Eksekusi Langsung (Tanpa create_task agar tidak jadi proses zombi)
        ...
        await msg_status.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Data JTII sudah tayang di sistem.",
            parse_mode='Markdown'
        )
        
    except Exception as e:
        error_msg = str(e)
        logger.error(f"Upload Fail: {error_msg}")
        await msg_status.edit_text(f"❌ **EKSEKUSI GAGAL!**\n⚠️ Penyebab: `{error_msg[:150]}`")
...
if __name__ == '__main__':
    ...
    app.add_handler(ConversationHandler(
        ...
        states={
            ...
            U_CONFIRM_UPLOAD: [MessageHandler(filters.TEXT & ~filters.COMMAND, upload_confirm_admin)] # Pastikan nama ini tunggal
        }, 
